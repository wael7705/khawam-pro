    ...
    <div className="order-notification-container">
      {notifications.map((notification) => (
        <div key={notification.id} className="order-notification-card">
          <button className="order-notification-close" onClick={() => onDismiss(notification.id)} title="إخفاء الإشعار">
            <X size={18} />
          </button>
          ...
          <div className="order-notification-body">
            ...
            <div className="order-notification-row">
              ...
              <div>
                ...
                <p className="value">
                  {notification.itemsCount ? `${notification.itemsCount} عنصر` : ''}
                  {notification.itemsCount && notification.finalAmount ? ' • ' : ''}
                  {notification.finalAmount ? `${notification.finalAmount.toLocaleString()} ل.س` : ''}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="order-notification-footer">
            ...
            {onViewOrder && notification.orderId ? (
              <button className="order-notification-action" onClick={() => onViewOrder(notification.orderId)}>
                فتح الطلب
              </button>
            ) : null
          </div>
